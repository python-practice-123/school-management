...
class TeacherServiceTest {
    ...
    @Test
    @DisplayName("should return teachers by subject")
    void getTeachersBySubject() {
        List<Teacher> teachers = List.of(mock(Teacher.class));
    ...
    void searchTeachersByName() {
        Page<Teacher> page = mock(Page.class);
        Pageable pageable = mock(Pageable.class);
        when(teacherRepository.findByFirstNameContainingIgnoreCase("John", pageable)).thenReturn(page);
        assertThat(teacherService.searchTeachersByName("John", pageable)).isEqualTo(page);
    }

    @Test
    @DisplayName("should return high performing teachers")
    void getHighPerformingTeachers() {
        List<Teacher> teachers = List.of(mock(Teacher.class));
        when(teacherRepository.findByPerformanceScoreGreaterThanEqual(90.0)).thenReturn(teachers);
